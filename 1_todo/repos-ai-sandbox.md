...
- ai sandbox session setup MUST include injecting unique GCP service account into a session pod


## IAM Setup - Providing credentials to sandbox session

- host during session creation MUST provide exactly one credential to session - GCP service account with policy binding allowing for reading secrets
- host must generate credentials for a session and create GCP secrets a session will read
- mechanism or script using for generating credentials MUST live in this repo
- sandbox session MUST fetch credentials from GCP and bootstrap access, using script that lives in this repo
- script MUST generate credential for each platform, and output only the credential
...
- terraform MUST be applied using user GCP identity on a host, never in CI
...
- all session credentials apart from GCP service account are published to GCP secrets manager
