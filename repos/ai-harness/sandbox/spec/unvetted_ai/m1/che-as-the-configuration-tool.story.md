...
Builds images from profiles. Configures nothing by hand.

### The build is reproducible from a profile (implemented)

I want che applying the image's configuration from a profile,
so that the build reads as source.

### Nothing lands by a hand-run command (todo)

I want tmux, credentials and remote rewrites applied by profile ops,
so that the Dockerfile holds no configuration.
...
I want the installs layer and the configuration layer running the same
profiles, each applying only its own ops,
