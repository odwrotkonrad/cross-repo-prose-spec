...
so that the host is unreachable from inside the sandbox.

### A host port is closed until a rule opens it (implemented)

I want a host port no rule names denied,
...
so that exposure is deliberate.

### Opening one port opens nothing else (todo)

I want a rule admitting one host port leaving every other denied,
so that a grant cannot widen by accident.
...
so that a one-way boundary costs nothing in workflow.
