...
Scenario: a single quoted argument runs as a shell line (implemented)
  ...
  Then the lone argument runs as `zsh -c <arg>` in each repo, so `;`, pipes, `&&`, globs work
  And multi-word invocations still run verbatim

Scenario: --include/--exclude select repos by name or path (implemented)
  When I run with `--include=a,b` and/or `--exclude=c,d`
  Then a token with `/` matches the root-relative repo path exactly
  And a bare token matches the repo dir basename
  And empty include means all repos, exclude applies after include
  And a basename matching more than one repo exits 2 listing the candidates
...
Scenario: --must-filter targets repos needing attention, AND semantics (implemented)
  ...
  Then only repos meeting every listed condition run
  And `changes`: `git status --porcelain` non-empty (tracked or untracked)
  And `off-main`: current branch is not `main`
  And `unsynced`: no upstream, or ahead/behind vs `@{u}` not `0 0`
...
Scenario: summary report closes the run, failures only (implemented)
  ...
  Then a bold `## Done <succeeded>/<count> <✅|❌> <clock> ✅ <n> ⏭️ <n> ❌ <n>` line closes the run, shaped like the Progress header, below the `## Passed` and `## Skipped` sections naming the repos behind those counts
  And failures follow under a bold `## Failed Executions` section, each a bold `### <repo> ❌ (exit N) <M>m<SS>s` block: `log: <log file>`, `tail:` + the log's 10 newest lines as blockquotes
