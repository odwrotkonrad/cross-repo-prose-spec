...
Scenario: User in another pane or window hears a terminal bell when claude finishes or waits for input
  Status: implemented
...
Scenario: User spots the waiting claude window at a glance in the tmux status bar
  Status: implemented
...
Scenario: User spots the exact waiting claude pane among many panes in one window
  Status: implemented
...
Scenario: User away from the terminal gets a macOS notification driven by the same bell
  Status: implemented
