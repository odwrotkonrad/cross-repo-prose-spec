...
(default 6), the query fuzzy-matching in every group, a history group last,
truncated to 4, a word holding `/` or `~` delegated to stock `_autocd` path
completion,
so that the first word completes from every source without flooding the screen.
