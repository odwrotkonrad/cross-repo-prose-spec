...
to their first character, the last two segments and the git repo root segment
full, the path dropped under tmux where the pane border shows it, the `$PWD`
group header listed as the glob-depth notation `*` and each stack entry
rendered with `$HOME` as `~`,
...
first character, the last two segments and the git repo root segment full,
