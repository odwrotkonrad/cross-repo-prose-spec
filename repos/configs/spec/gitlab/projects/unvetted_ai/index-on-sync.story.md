...
### Sibling repos' purposes are rendered before the routine index (todo)

I want the routine sync rendering every cloned repo's purpose doc before
indexing,
so that no sibling repo appears in the map without its purpose.

### The full sync keeps its existing behavior (implemented)
...
### Inventory describes files this repo holds (implemented)
