...
Scenario: the publish path is testable before it merges
  ...
  And gate jobs missing from such a pipeline are tolerated, since each `needs` is optional
...
Scenario: publishing costs the pipeline no new job and no new secret
  ...
  And it authenticates with `CI_JOB_TOKEN`, the header the tag-pipeline publish scripts already use
  And it reuses the retry flags the other publish scripts carry

Scenario: a prerelease is gated on every automatic test, and nothing slower
  Status: implemented
  ...
  And it never waits on `test-e2e-che-registry`, which asserts upstream package names, not the shipped binary
...
Scenario: a merge request holds one prerelease, always its newest build
  Status: implemented
...
Scenario: an operator reads which platforms a prerelease covers
  Status: implemented
