...
An MR pipeline exists to tell a developer, quickly, whether a change is sound and
...
every job compiles the same 1422-package dependency tree from cold. Measured on
one merge-request pipeline, `test-unit-che` spent 165 seconds compiling before its
first test ran, and the three che e2e jobs each waited about eleven minutes to do
three to nine seconds of work.

This file covers the shape of the job graph: what waits on what, and why. The other
half of the fix is the build cache, specified in
[pipeline-caching.md](pipeline-caching.md); provisioning it is infrastructure and lives
in
...
Scenario: unit tests start immediately instead of queueing behind a build they never use
  ...
  And the other per-module unit jobs likewise start without waiting
...
Scenario: e2e jobs stop waiting eleven minutes to do nine seconds of work
  ...
  And `warm-go` reaches them substantially sooner, having shed the work they do not use
...
Scenario: speed is sought where cores are added, not where they are only shared
  ...
  Then the work is spread across more jobs rather than backgrounded within one
  And no job is restructured to run its own steps concurrently against cores it does not have
  And `Unschedulable: Insufficient cpu` waits are understood as the node-packing described in
