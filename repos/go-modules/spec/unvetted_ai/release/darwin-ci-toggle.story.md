...
Controls darwin CI cost across every repo. Edits no per-repo pipeline for it.

### Darwin cost stops without deleting the jobs (implemented)

I want the darwin test and dry-run jobs gated on `ENABLE_DARWIN_CI == "true"`,
definitions left in place,
...
terraform,
so that no repo carries a copy and one change reaches all pipelines.

### The pipelines it gates can read it (implemented)

I want the variable neither protected nor masked, a behavior flag, not a
secret,
so that MR-branch pipelines evaluate their rules against it.

### Coverage returns with one variable (implemented)

I want `"true"` to run every gated darwin job as before the toggle existed,
...
so that no job appears to have passed without running.

## As a release consumer

Installs che binaries. Does not care which runners built them.
...
so that a release or prerelease never lacks darwin archives over a cost flip.
