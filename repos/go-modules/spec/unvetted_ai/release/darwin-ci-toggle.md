...
hardware on GCP). One variable, `ENABLE_DARWIN_CI`, turns them off while the cluster is
proven, and back on in one flip. It is a **group-level** CI variable on `konradodwrot`,
declared in `infra/iac` terraform, so one flip governs every repo. Release-publishing
darwin jobs are exempt: disabling them would ship che without darwin binaries.
...
Scenario: darwin CI cost stops without deleting the jobs
  ...
  And their definitions remain in the CI config, gated rather than removed
...
Scenario: one flip governs darwin CI across every repo
  ...
  Then every repo's pipelines observe the new value
  And no repo carries its own copy of the toggle

Scenario: the toggle is readable by the pipelines it gates
  Status: todo
  Given the toggle is a behavior flag, not a secret
  When an MR-branch pipeline evaluates its rules
  Then the variable is readable there, being neither protected nor masked
...
Scenario: darwin coverage returns with one variable
  ...
  Then every gated darwin job runs exactly as before the toggle existed
...
Scenario: a tag pipeline still publishes when darwin CI is off
  ...
  And a `needs:` on a rules-excluded job would otherwise stop the pipeline being created
...
Scenario: a release without darwin binaries is an explicit choice, not a surprise
  Status: todo
  ...
  And cutting a release intended for darwin users requires enabling darwin CI first
...
Scenario: a reader can tell why a darwin job was skipped
  ...
  And no job appears to have silently passed without executing
