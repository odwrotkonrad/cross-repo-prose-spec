...
package registry from inside the same job, so a che change is installable before
it is ever tagged. Linux covers amd64 only, darwin arm64.
...
`ci/publish-prerelease.zsh`, the `Makefile` or `.gitlab-ci.yml`, tolerating
absent gate jobs through optional `needs`,
so that a change to publishing is tested before it merges, not after.

### A merge request always holds its newest build (implemented)

I want the version keyed on the iid and never the commit, re-uploads replacing
...
Owns the prerelease jobs and their gates. Adds no jobs and no secrets.

### Publishing costs no new job and no new secret (implemented)

I want the upload as a final step of the existing job, authenticated with
`CI_JOB_TOKEN` and reusing the existing retry flags,
...
`test-e2e-che-{dryrun,run,backup}` jobs, and never on
`test-e2e-install-methods`, `test-e2e-che-registry` or the coverage stage,
...
I want the linux job uploading only from `dist/` and the darwin job only from
...
as the tag pipeline already does, with merge-request build caches kept separate
from release caches,
