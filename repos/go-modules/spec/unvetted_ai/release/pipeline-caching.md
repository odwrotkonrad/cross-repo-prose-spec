...
Every job in this pipeline compiles the same 1422-package dependency tree — gomplate's
transitive AWS, GCP, go-git and OTel trees dominate it. Measured on one merge-request
pipeline, `test-unit-che` spent 165 seconds compiling before its first test ran, and
`warm-go` spent 202 seconds archiving caches that, with no distributed cache
configured, nothing could ever restore.

Only compiler output is cached. Dependency downloads are refetched from the public
module proxy every time: they are the larger half of the cache but the cheaper half of
the work, so they are left out deliberately.

Provisioning the cache is infrastructure and lives in
[infra/iac RunnerBuildCacheBehavior.md](../../../../infra/iac/spec/unvetted_ai/ci-cluster/RunnerBuildCacheBehavior.md).
That module guarantees only that entries persist, are reachable, and expire; it offers
no isolation between keys. This file covers what this repo decides: what is cached,
...
Scenario: a developer's unit results arrive without recompiling the world first
  Status: todo
  ...
  And it begins running tests without first compiling che's full dependency tree
...
Scenario: only the expensive half of the work is cached
  ...
  Then it caches compiler output, the part that is expensive to recreate
...
Scenario: a stale compiler output is never served for changed input
  ...
  And a cache key that is a static string is safe for build output, because Go does this keying itself
