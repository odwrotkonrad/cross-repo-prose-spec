...
script, pyenv, nvm, nix), the first applicable on this host wins. Profiles
declare `include.installPackages`, the run sequence installs them before
`runScripts`. Four check subcommands report presence, upgradability, shadowing,
duplicates.

## As a catalog author

Writes the packages file. Declares what a package is and how each manager
spells it, never how installs execute.

### A package named once, by its CLI name, resolving everywhere (tested)

I want a canonical name (the CLI program name where the package ships one) to
install on any supported host, bare installer items reusing it,
`installerVocabulary.packageName` overriding per installer, and casks as the
`brew/cask` installer key with vscode extensions living in `toolPackages.vscode`
instead of an installer key,
...
carrying binariesRemoteArchive, script, npm, go, gem, pyenv, nvm, nix,
linux-debian adding apt, darwin adding brew and brew/cask), a method outside the
set being a hard error naming the valid set,
so that a typo in a method name never installs nothing quietly.

### Managers listed in preference order, first applicable winning (tested)

I want the first applicable item in entry order to install, brew/cask applying
on macos with brew present, apt on linux with apt-get, go and gem where their
command exists, npm applying even without the command when nothing else is
strictly applicable, binariesRemoteArchive when platformEligibility carries this
os-arch, an unknown manager and an unknown package name both hard errors, a
requested package with no applicable method a hard error
(`--missing-method-warn` downgrading it) and a dependency-pulled one a logged
skip,
so that fallbacks are expressed by list order alone.
...
managers running their update path), embedded pins in item names to be parse
errors naming the version field, and install and dry-run messages to label the
package with its pinned version,
...
an entry-level version stated only when every installer honors it, item-level
versions beating it, `latest` meaning no pin, an unenforceable version warning
and installing the current release (brew/cask: casks are rolling), and the old
`__rolling__` sentinel being a parse error,
so that rolling is expressed by omission, not a sentinel.
...
requested, none being a hard error), a drifted manager-installed package to run
the manager's update path with check-upgradable warning, and entries whose
manager ships one recent version to stay unpinned by convention while the
builtin pins every version-distributing entry,
so that one field states the version the host must reach.

### A pinned brew item deriving its versioned formula (tested)

I want a pin to install `<name>@<pin>` with the suffix derived and never
written, a literal version suffix in `installerVocabulary.packageName` a parse
error, and an unpinned item installing the bare formula,
...
comparing dpkg's version and check-upgradable probing for the binary version,
the map holding exactly one pair and the item naming exactly one deb via
...
slug) even when the package is present so a pruned repo file heals, a
`verificationKey` accepting a url or an absolute path, explicit `suites:`
installing with `-t <suites>` so exact-version dependencies resolve, and an
undeclared registry or one missing url or verificationKey a hard error,
...
`fromRegistry: cirruslabs/cli`, installing to tap the repo, trust the
tap-qualified name and install, a tap-qualified `packageName` a parse error
...
sources declared under `installerRegistries.nix` and picked by `fromRegistry`
(defaulting to `nixpkgs`, an undeclared one a hard error), unpinned items
installing `nix profile install <url>/<ref>#<attr>` with install lines naming
the registry and `--update` running `nix profile upgrade`, presence and version
read from `nix profile list` with a `--version` fallback, and a packageName
carrying `#`, `@` or `=` plus `platformEligibility`, `extractBinaries` and
...
comparing the profile's store-path version, a drifted install running
`nix profile remove <attr>` then installing the pinned ref, a pinned version
with no revision in the versionMap a hard error naming versionMap, and every
builtin nix item pinned to its registry's channel head with one rev per
registry,
so that a nix pin is reproducible rather than a name lookup.
...
winning and only its installers applying with a fallback to built-in rules, apt
eligible only under `linux-debian` (distro read from /etc/os-release ID),
`platformEligibility` ids validated as `<os>-<arch>` against both blocks with an
...
`{arch}`, and the old `{arch_x}`/`{arch_g}` tokens and per-item `archNames:`
maps parse errors,
...
against the platform's algorithm-prefixed checksum (an unprefixed value a parse
error, a bare platform name skipping verification with a warning, a mismatch
aborting), `.tar.*` extracting the listed members, `.zip` unzipping, bare assets
installing as-is, a pinned version absent from the probe output triggering
reinstall, and the probe running `<canonical> --version` falling back to
`version` unless `versionCommand:` says otherwise,
...
resolving relative to the packages file, `url:` fetched with retrying curl and
aborting on failure or empty body, a present command skipping the script,
optional `version:` and `platformEligibility` pinning declaratively (exporting
CHE_PKG_VERSION, CHE_PKG_SHA256 as bare hex, CHE_PKG_NAME/OS/ARCH and
CHE_PKG_ARCH_<SET>) with a `--version` output lacking the pin reinstalling and
check-upgradable warning, and dry run announcing `install <pkg> via script`
without executing,
...
I want a relative script `path:` in a superseding or override packages file
(an install method's or `postInstall`'s) to resolve against that file's
...
`# yaml-language-server: $schema=` modeline pointing at it and the builtin
carrying that modeline against the published url,
...
I want resolution to run in rounds so a package needing npm installs in a later
...
ensured once each per run, the npm invocation resolving through the nvm default
node's bin dir when npm is off PATH, and `--only-methods npm` not filtering the
bootstrap,
...
I want dependency lines labeled `<requirer> dependency <pkg>`, spec-requested
packages keeping their bare name, a reinstall line showing the currently
installed version when known, an unpinned apt installed line reporting the
installed version and an updated line reporting the manager's post-update
version,
so that the log says why each package is being installed.
...
I want `--if-missing` to skip any package whose canonical command exists
...
with no manager command executed,
...
pick the first candidate found on PATH, warn once per run listing them and use
the first entry when none is, and `checkPresentOnPath: false` to skip the probe
and always use the first,
so that a binary never lands somewhere the shell cannot reach it silently.
...
through, the cascade running flag/env > profile > spec > user config and an
unknown method name a hard error naming the valid set,
...
install of the run, at most once and re-armed when a new apt registry is
...
checksum mismatch deleting the cached file before failing and an empty value
...
so that a green install means the tools are actually callable.
...
so that two competing installs are found before they diverge.
...
`packages:` being a parse error pointing at `toolPackages.vscode`,
