...
### Naming the catalog source without editing che (todo)

I want `packages.source.url` to accept the location definitions are read from,
cascading profile over spec over user config like every other packages option,
so that a fork, a mirror or a private catalog needs no fork of che.

### One key covering both ways a catalog is published (todo)
...
### The default source naming the catalog's real home (todo)
...
### Holding a host to an exact catalog version (todo)

I want `packages.source.ref` to name a published version or a git ref, che
fetching exactly that and nothing newer,
so that a spec pinning its catalog installs the same definitions every run.

### Reading the conditions off the config (todo)
...
### The newest catalog without asking for it (todo)
...
### One update check per che run, whatever the spec holds (todo)
...
### A pinned run never waiting on a cooldown (todo)
...
### A dry run that works with the network gone (todo)
...
### A failed fetch never being fatal (todo)
