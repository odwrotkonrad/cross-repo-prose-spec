...
Runs che on a real host, recovers it when a run goes wrong. Owns the machine,
not che's code.
...
skip settled ones,
so that an archive holds only the state at risk.
...
`# backups` heading, newest first, with run id, backup id, timestamp, size and
abbreviated path,
...
I want `backup restore --backup-id <id>` to restore only the archive carrying
that id and fail clearly on an unknown id,
so that one file recovers without rolling back its neighbours.

### The host rolled back to a chosen moment (tested)

I want `backup restore --timestamp <ts>` to restore each dest to its newest
backup at or before the timestamp, leave dests with no such backup as-is, and
fail clearly when nothing predates it,
so that recovery targets a point in time, not a run id I have to hunt for.

### Pre-run state recovered, drifted files never clobbered (tested)

I want restore to demand exactly one of `--run-id`, `--backup-id`,
`--timestamp`, skip dests drifted from che's last recorded state, print
`restore <dest> (dry run)` per entry under dry run, and fail clearly on an
unreadable archive,
...
run's archives sharing timestamp and run id, each with a unique 12-char backup
id,
so that a path names profile, op and run with no lookup.
...
ops, a `created <size>, <path>` line per written archive, nothing more when
there is nothing to back up, and `create <path> (dry run)` under dry run,
so that the archive is auditable from the log alone.

### Proof the backup ran even with nothing to archive (tested)

I want a `## backup` heading under the profile heading, the delta line always
beneath it,
so that a silent stage is never mistaken for a skipped one.

## As a developer

Works on che's ops and ledger. Owns the run sequence, not the host.
...
archive while automatic backup is on,
so that no mutation precedes its own recovery point.
...
`run` stage and a direct op's own archiving,
so that a throwaway CI container skips work discarded with it.
...
so that turning off automatic protection never disarms the command whose job
is protection.
...
so that a record always resolves to the archive that undoes it.
