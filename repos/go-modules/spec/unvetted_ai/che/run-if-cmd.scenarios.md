...
Scenario: a config author gates a profile on a command that succeeds (todo)
  Given a profile with `runIf: [cmd:<argv>]`
  When the command exits 0
  Then the predicate passes and the profile is eligible

Scenario: a failing gate keeps a profile out of the run (todo)
  Given a profile with `runIf: [cmd:<argv>]`
...
Scenario: a missing binary skips instead of aborting the run (todo)
...
Scenario: a gate over mutable state is never answered from cache (todo)
  Given a profile with `runIf: [cmd:<argv>]`
...
Scenario: a decision-only probe leaves the output clean (todo)
...
Scenario: a command gate composes with the existing literal form (todo)
...
Scenario: every predicate must pass before a profile runs (todo)
...
Scenario: an unknown source names the sources it accepts (todo)
...
Scenario: spec validation accepts a command predicate (todo)
