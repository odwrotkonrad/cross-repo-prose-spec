...
Scenario: a host reads definitions from a source it names (todo)
...
Scenario: a catalog published as a git repository is usable as-is (todo)
...
Scenario: an unset source resolves to the catalog's own repo (todo)
...
Scenario: a pinned ref installs the same definitions every run (todo)
...
Scenario: an unpinned run picks up the newest catalog (todo)
...
Scenario: repeated unpinned runs stay cheap (todo)
...
Scenario: a multi-profile apply makes one round-trip (todo)
...
Scenario: an explicit update always acts (todo)
...
Scenario: planning offline is a config choice (todo)
...
Scenario: a dead source costs a warning, not a run (todo)
