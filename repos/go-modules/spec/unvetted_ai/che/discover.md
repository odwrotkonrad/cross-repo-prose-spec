...
Scenario: discovery costs once per run, not per wrapped command
  Status: tested
  When I invoke `run`
  Then discover runs once, not per wrapped command

Scenario: a config author previews a run without touching the host
  Status: tested
...
Scenario: a directly invoked op still prints the full plan first
  Status: tested
  Given a profile whose runtime spec contains os-mutating operations
...
Scenario: every wrapped command works from one discovery
  Status: tested
...
Scenario: the discovery plan prints with no log-level tuning
  Status: tested
