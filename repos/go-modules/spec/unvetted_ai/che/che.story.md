...
Runs che over profiles on a host. Reads the output, not the code.

### Profiles run exactly as the discovery log lists them (tested)

I want discovery order to be execution order,
so that the printed plan is the run.

### A profile with nothing to change costs no time (tested)

I want a zero-delta profile skipped wholesale with a debug
...
I want `run.skipOps` to bind to the `run` sequence alone, direct op
subcommands executing normally even while listed there,
so that an op invoked by name is never silently ignored.
...
I want a `## Profile <ref>` heading before a profile's ops log,
...
naming only changing dests and all's every dest, no other line marked,
`--dry-run=true` aliasing delta,
...
script reported `ran` or `failed`, failures joined into the final error,
so that one run surfaces every broken script.

### Errexit aborts at the first script failure (tested)

I want `--errexit` (or `CHE_ERREXIT`) to stop remaining scripts, ops and
