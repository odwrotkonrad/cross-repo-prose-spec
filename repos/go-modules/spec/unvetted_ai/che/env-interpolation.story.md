...
values into any scalar of any che.yml: local, included or sourced. Mapping
keys stay literal. Unset bare refs follow `options.envUnset: error | empty`,
default `error`. `$VAR` dest expansion and `runIf: ['env:NAME']` are untouched.

Lookup precedence at load, most specific last: the root spec's `.env`, the
process env, the referenced spec's top-level `env:`, the sourcing ref's `env:`
overlay. Same order `assembleProfiles` applies at runtime (`docs/spec.md`: the
referenced spec's `env:` merges under the ref's).
...
one env. Fetching values by running a tool at render time:
...
options, package lists) replaced with the launch env's value, several refs and
literal text allowed in one scalar,
so that one spec serves many invocations without forks or profile branches.

### The repo's .env is read without a shell reload (tested)

I want che to source `.env` beside the root che.yml (dotenv lines, `export`
prefix tolerated, missing file fine) beneath the process env, a shell export
always beating the file,
...
unset or empty, never erroring under any policy, the default taken literally
(everything after the first `||` up to `}}`, trimmed, never itself
interpolated),
...
the spec, each var with its YAML path (`/web/include/makeLinks/0`), and the
...
I want an unset bare ref to fail only inside a profile selected for the run
(`--profiles`, auto-discovery, a sourced ref), profiles skipped by selection
or `runIf` never erroring, top-level keys outside any profile (`env:`,
`options:`) always strict,
so that `che run --profiles a` never fails over a var only profile `b` needs.

### Unset tolerated when the spec says so (tested)

I want `options.envUnset: empty` (cascade: flag `--env-unset`, env
`CHE_ENV_UNSET`, user config, spec, default `error`) to resolve unset bare refs
to the empty string, the option read from the raw file before interpolation so
it is always literal, like `validateSpec`,
so that optional parameterization needs no fallback on every site.

### The spec's env block feeds its own interpolation (tested)

I want top-level `env:` values available to `${{ env.NAME }}` in the same
file, beating the process env, each `env:` value expanded one level from the
env passed into the load (process env plus any ref overlay), never from
sibling keys,
...
I want substituted scalars to stay strings (`'${{ env.PORT }}'` never an int,
`'${{ env.FLAG }}'` never a bool) and mapping keys (profile names) literal,
...
every other `${{`, `$VAR`, `${VAR}` or `env.X` passed through,
...
I want a sourced ref's `env:` overlay visible to the referenced spec's own
`${{ env.NAME }}` refs at load, beating both the process env and the
referenced spec's `env:` block,
so that one shared profile renders per consumer without edits, the more
specific site having the last word.
...
against its own `env:` block plus its ref's overlay, through the one load path
every spec takes,
...
`${{ env.* }}` ref, its default if any, and whether it is set, unset
requirements reported rather than failing discovery,
...
profiles running sequentially, so `env.Getenv "NAME"` resolves in any
template,
