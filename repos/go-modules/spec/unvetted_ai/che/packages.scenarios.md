...
Scenario: --update refreshes everything (tested)
...
Scenario: --only-methods restricts installs to chosen methods (tested)
...
Scenario: binariesRemoteArchive downloads cache across runs (tested)
...
Scenario: an install run ends proving the commands exist (tested)
...
Scenario: packages.yml ships a json schema of its structure (tested)
