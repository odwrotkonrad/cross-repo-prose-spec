...
so that talking to the user is not bound to opening an MR.

### Posting never stalls the work (todo)

I want to keep working when a posted message goes unanswered,
so that the channel is async and the session is never blocked on a human.
...
observation worth a look, and a notice naming a configuration change useful to
others,
...
so that blocked work is visible instead of silently abandoned.
...
I want it to carry messages from sessions to the user with no route back into a
session,
