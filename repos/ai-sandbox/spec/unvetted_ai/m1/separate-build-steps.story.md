...
### Three layers, three entry points (todo)
...
### A setting change rebuilds seconds, not minutes (todo)
...
### A rebuilt configuration reuses the tools beneath it (todo)
...
### Adding a tool rebuilds only from that layer up (todo)
...
### A base change does not disturb what sits above (todo)
...
### A sandbox is buildable with no CI at all (todo)
