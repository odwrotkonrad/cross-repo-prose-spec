...
### Everything is writable (todo)

I want a write to any path the configuration image provides succeeding,
so that no path is off limits mid-task.

### What must outlive the pod does (todo)

I want the workspace intact and scratch paths back to the image's content after
a stop and start,
...
### Nothing is mounted from the host (todo)
...
### One session's writes stay its own (todo)
