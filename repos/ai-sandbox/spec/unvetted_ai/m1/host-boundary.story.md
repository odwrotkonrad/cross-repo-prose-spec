...
### Escaping a session gains no foothold on the machine (todo)

I want a session denied on every host port,
so that the host is not reachable from inside the sandbox.

### A host port is closed until a rule opens it (todo)

I want an unnamed host port denied and an admitted one allowed,
...
### The session is still reachable to work in (todo)
