...
### A missing tool arrives by its profile (todo)

I want a requested tool installed by the configs profile that defines it, never
by a hand-run command,
so that the install is repeatable in the next image.

### The session feels like the host (todo)
...
### A configs change reaches sessions untouched (todo)
