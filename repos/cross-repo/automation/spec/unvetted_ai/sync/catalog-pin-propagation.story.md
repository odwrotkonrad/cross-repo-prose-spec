...
was the problem: `release-che` fires on `changes: [che/**/*]`, so raising the
pin cut a che release with no che change. The tag stream tracked the catalog
instead of che, and every che consumer paid a version bump for data it could
have fetched.
...
CI pins by setting an environment variable, a local run leaves it unset and
floats to latest.

The variable still has to move when the catalog publishes.
[One place carries versions between repos](version-propagation.story.md) says
how: a catalog tag triggers automation, automation evaluates the graph, the
repo owning group configuration applies the raise. The pin lives in terraform,
the only place group CI variables are written.
...
I want the release build to vendor exactly the pinned version, not the newest,
...
edit in go-modules,
so that the binary's offline fallback stays current without a pin-bump commit.
...
so that a pipeline run never depends on what the registry published mid-run.
...
I want a catalog tag to trigger automation, which evaluates the graph and
drives the raise in the repo owning group configuration,
so that nobody raises the pin by hand or has to remember it exists.
...
so that onboarding the catalog needs no special case in automation.
