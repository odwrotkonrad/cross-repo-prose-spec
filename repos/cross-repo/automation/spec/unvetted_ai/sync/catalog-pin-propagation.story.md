...
### A catalog bump that releases no binary (todo)
...
### A build still embedding an exactly known catalog (todo)
...
### The embedded catalog rising with the pin, on the next release (todo)
...
### One variable both CI and che already understand (todo)
...
### A local run floating where CI pins (todo)
...
### CI updating definitions only when the pin says so (todo)
...
### A catalog release reaching the pin automatically (todo)
...
### The catalog appearing in the graph like any producer (todo)
...
### Group variables changing only through terraform (todo)
...
### A raise being a reviewable change (todo)
