...
Pays the GCP invoice. Wants warning before it arrives.

### Overspend surfaces before the invoice (implemented)

I want a $100 budget on the whole billing account, notifying at 50% and 100%,
so that total GCP exposure across every project shows early.

### A bad trend warns while it can still be stopped (implemented)

I want a forecasted-spend threshold beside the actual ones,
so that a projected month-end overrun notifies with time to act.
...
so that raising the ceiling or changing the recipient is a value change.
