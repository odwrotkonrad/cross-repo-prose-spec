...
### Throwaway builds skip work whose product dies with them (todo)
...
### che's own tests never read the switch (todo)
...
### Opting out is per job, never a repo-wide default (todo)
...
### A real host keeps its recovery point (todo)
...
### An explicit snapshot still works where it is asked for (todo)
...
### One declared home for the CI value (todo)
