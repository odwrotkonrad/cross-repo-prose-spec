...
Before mutating a dest, che archives it, so a bad run reverts. On a real host
that archive is the whole safety net. In a container thrown away minutes later
it protects nothing: the tarball dies with the layer, after a bz2 pass over
every file the run touched.

che reads `backup.autoCreate.enabled`, default true. A group variable carries
the CI value. Each pipeline decides which of its jobs read it.

Per job, never group-wide by default. che's own suites assert that backups
happen: a variable reaching them turns a passing test red, or worse, green
...
backup off,
so that no pipeline minute goes to compressing files discarded with their
layer.
...
so that the suites asserting backup behaviour keep exercising it and cannot
pass against a disabled feature.

### Opting out is per job, never a repo-wide default (implemented)

I want the remap declared on the jobs that want it, not in a repo's top-level
`variables:` block,
so that a new test job never silently inherits a disabled backup.
...
automatic backup on,
so that a bad apply stays revertible wherever reverting is possible.
...
so that a job wanting a recovery point gets one by asking, whatever the
automatic default.
