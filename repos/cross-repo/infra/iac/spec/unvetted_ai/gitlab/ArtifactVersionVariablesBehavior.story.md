...
consumer, sed-bumped by regen MRs. `che_packages_ref` already escaped that: one
tfvars line, one `GRP_KO_VAR_CHE_PACKAGES_REF` group variable, one line to bump.
Every artifact gets the same, prose (`prose_ref`, `GRP_KO_VAR_PROSE_REF`) first.
...
unmasked and unprotected (a version string, MR pipelines need it), consumers
deriving the bare name at the pipeline boundary,
