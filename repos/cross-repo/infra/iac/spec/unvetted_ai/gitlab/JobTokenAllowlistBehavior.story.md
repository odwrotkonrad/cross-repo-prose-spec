...
I want prose declared in control's job token allowlist under
instance-enforced inbound scope,
so that a prose tag pipeline creates the downstream pipeline rather than
...
so that permitting another project's token changes only that entry, never a
resource definition.

### Terraform never fights the instance (implemented)

I want the module to add a single allowlist entry and never write the
scope-enabled flag,
...
so that permitting prose to reach control grants control nothing back and no
third project access as a side effect.
