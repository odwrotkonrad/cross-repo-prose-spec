...
derives the unprefixed name from the group variable per the ci-variables
convention, hosts seed it into `.env` (`dev-env/upstream-refs.story.md`). No
fallback version anywhere: an unset
ref fails the load and names itself. prose (`PROSE_REF`) is the first
artifact, the pattern holds for all.

## As a repo owner

Pins upstream artifacts in the repo's che specs and templates. Does not track
their release streams.
...
### CI derives the unprefixed name at the pipeline boundary (implemented)

I want each pipeline that consumes an upstream to assign
`<ARTIFACT>_REF: $GRP_KO_VAR_<ARTIFACT>_REF` in one reviewable line in its
top-level `variables:` block, the unprefixed name never defined in GitLab,
so that the value has one source and its scope is readable at the point of
use.
