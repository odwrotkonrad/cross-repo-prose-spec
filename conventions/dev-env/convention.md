...
- Dependencies install **into the checkout**, never system-wide: prepare needs no
  root. Where the tool defaults to a system path, the step sets the local one
...
- Only real prerequisites go in the chain. A target producing build artifacts is
  an output, not a prerequisite: preparing is not building. Nor does prepare touch
  remote state or credentials: installing providers is preparation, initialising
  a state backend is not.
- Prefer a tool already on `PATH` over building one. A repo that can build its own
  toolchain does so only when none is installed, so a fresh clone never waits on
  a compile.
...
one broken checkout cannot abort a session bootstrap or an image build.
